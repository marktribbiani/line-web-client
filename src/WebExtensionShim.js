    ...
    search: function (query, callback) {
      if ('startTime' in query) {
        var result = historyItemsJson.filter(
          (item) => !('lastVisitTime' in item) || item.lastVisitTime > query.startTime,
        );
      } else {
  ...
  runtime: {
    ...
    getPackageDirectoryEntry: function (callback) {
      r = {
        ...
        filesystem: {
          name:
            'chrome-extension_ophjlpahpchlmihnnnihgmmeilfjmjjc_0:Isolated_C7048C435E514A1E8E476314FA6BDC71',
          root: {},
        },
      };
      callback(r);
...
(() => {
  ...
  XMLHttpRequest.prototype.open = function (...args) {
    if (args[1].startsWith('https://')) {
      const url = `https://linewebapi.com/?url=${encodeURIComponent(args[1])}`;
      open.call(this, ...args.slice(0, 1), url, ...args.slice(2));
      return;
    }
    open.call(this, ...args);
  };
  ...
  const theXMLHttpRequest = window.XMLHttpRequest;
  window.XMLHttpRequest = function (...args) {
    const xhr = new theXMLHttpRequest(...args);
    xhr.timeout = 10000;
    return xhr;
  };
})();
